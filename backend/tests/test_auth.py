...
def test_register_returns_token_and_public_user(client):
    res = client.post("/api/auth/register", json={"email": "Zoe@Example.com", "username": "zoe", "password": "password123"})
...
def test_register_missing_field_returns_readable_422(client):
    res = client.post("/api/auth/register", json={"email": "zoe@example.com"})
...
def test_register_rejects_duplicate_email(client, alice):
    res = client.post("/api/auth/register", json={"email": "ALICE@example.com", "username": "alice2", "password": "password123"})

    assert res.status_code == 409


def test_register_rejects_duplicate_username_case_insensitively(client, alice):
    res = client.post("/api/auth/register", json={"email": "other@example.com", "username": "Alice", "password": "password123"})
...
def test_login_with_username_or_email(client, alice, identifier):
    res = client.post("/api/auth/login", json={"identifier": identifier, "password": "password123"})
...
def test_me_returns_current_user(client, alice):
    res = client.get("/api/auth/me", headers=alice.headers)
...
def test_logout_revokes_token(client, alice):
    res = client.post("/api/auth/logout", headers=alice.headers)
...
def test_logout_only_revokes_that_session(client, alice):
    second = client.post("/api/auth/login", json={"identifier": "alice", "password": "password123"}).json()["token"]

    client.post("/api/auth/logout", headers=alice.headers)
